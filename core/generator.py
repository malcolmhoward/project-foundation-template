...
class EthicalFoundationGenerator:
    ...
    def check_existing_files(self):
        """Check for existing PFT-generated files and warn before overwriting.

        This safeguard prevents accidental loss of customizations made to
        previously generated files.

        Returns:
            True if generation should proceed, False to cancel
        """
        output_dir = Path(self.args.output_dir)

        if not output_dir.exists():
            return True  # No existing directory, safe to proceed

        # Files that PFT typically generates
        pft_files = [
            "README.md", "CONTRIBUTING.md", "LICENSE", "CODE_OF_CONDUCT.md",
            "SECURITY.md", "CHANGELOG.md", ".gitignore", "FOUNDATION_NOTICE.md",
            "SCAFFOLD_MANIFEST.md", "GLOSSARY.md", "MAINTAINERS.md",
            ".pre-commit-config.yaml", "GENERATION_LOG.md", "GENERATION_LOG.json"
        ]

        pft_dirs = [
            ".github/ISSUE_TEMPLATE", ".github/workflows", "docs/adr", "scripts"
        ]

        # Check for existing files
        existing_files = []
        for fname in pft_files:
            fpath = output_dir / fname
            if fpath.exists():
                existing_files.append(fname)

        for dname in pft_dirs:
            dpath = output_dir / dname
            if dpath.exists():
                existing_files.append(f"{dname}/")

        if not existing_files:
            return True  # No existing PFT files, safe to proceed

        # Force overwrite flag bypasses the check
        if getattr(self.args, 'force_overwrite', False):
            if not self.is_quiet:
                print(f"⚠️  Overwriting {len(existing_files)} existing file(s) (--force-overwrite)")
            return True

        # In non-interactive mode without force flag, warn but proceed
        if not self.is_interactive:
            print(f"⚠️  Warning: {len(existing_files)} existing file(s) will be overwritten.")
            print("   Use --force-overwrite to suppress this warning.")
            return True
...
The output directory contains {len(existing_files)} file(s) that may have been
previously generated by PFT:
...
Regenerating will OVERWRITE these files. Any customizations you've
made to these files will be lost.

Options:
  [P] Proceed - Overwrite existing files
  [C] Cancel - Stop and preserve existing files

Tip: Use a different --output-dir to generate alongside existing files.
""")

        while True:
            try:
                choice = input("Your choice [p/C]: ").strip().lower()
                if choice in ("", "c"):
                    print("Generation cancelled. Existing files preserved.")
                    return False
                elif choice == "p":
...
""")
    ...
    def generate_generation_log(self):
        ...
        The generation log provides project-level transparency about which files
        were generated and when. It complements the usage logging safeguard
        defined in ETHICS.md#3-usage-logging.

        This method includes overwrite protection: if an existing generation log
        is detected, the user is prompted to append, overwrite, or cancel.
        """
        ...
        force_overwrite = getattr(self.args, 'force_overwrite', False)
        ...
        created_logs, success = write_generation_log(
